    ...
    private fun setupViewModel() {
        ...
        mainViewModel.isShowProgress.observe(this) {
            ...
            if (it) {
                ...
                progressDialog?.setTitle("测试延迟进度")

                progressDialog?.setCancelable(false) //设置是否可以通过返回键退出此布布局
                progressDialog?.show()
            } else {
                progressDialog?.dismiss()
            }
        }
        mainViewModel.currentProgressIndex.observe(this) {
            Log.d(TAG, "currentProgressIndex: $it")
            progressDialog?.setMessage("Progress: " + it.toString() + "/" + mainViewModel.serverList.count())
        }
...
//        }
        ...
        R.id.currentPosition -> {
            Log.d("TAG", "onOptionsItemSelected: currentPosition:" + adapter.currSelectedIndex)
            adapter.currSelectedIndex?.let { binding.recyclerView.scrollToPosition(it) };
            true
        }
